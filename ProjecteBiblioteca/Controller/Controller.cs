...
namespace Controller {
    public class Controller {
        ...
        public void initListeners() {
            ...
            BibliotecaAdmin.buttonAbout.Click += aboutProgram;
            ...
            BibliotecaAdmin.autor1.textBoxBuscarAutor.TextChanged += buscadorAutor;
            ...
            BibliotecaAdmin.modificarAutor1.checkBoxSwitchLlibres.CheckedChanged += switchLlibres;
            BibliotecaAdmin.modificarAutor1.buttonAfegirLlibres.Click += afegirLlibreToAutor;
            BibliotecaAdmin.modificarAutor1.buttonEliminarLlibres.Click += eliminarLlibreToAutor;
            ...
            BibliotecaAdmin.llibre1.textBoxBuscarLlibre.TextChanged += buscadorLlibre;
            ...
            BibliotecaAdmin.modificarLlibre1.checkBoxSwitchAutors.CheckedChanged += switchAutors;
            BibliotecaAdmin.modificarLlibre1.buttonAfegirAutors.Click += afegirAutorToLlibre;
            BibliotecaAdmin.modificarLlibre1.buttonEliminarAutors.Click += eliminarAutorToLlibre;
            ...
            BibliotecaAdmin.usuari1.checkBoxMostrarBaixa.CheckedChanged += veureUsuarisBaixa;
            // Prestec
        ...
        public void aboutProgram(object sender, EventArgs e) {
        ...
        public void buscadorAutor(object sender, EventArgs e) {
            String text;
            if ((text = BibliotecaAdmin.autor1.textBoxBuscarAutor.Text) != "") {
                autorsPopulateBuscador(text);
            } else {
                autorsPopulate();
            }
        }

        public void buscadorLlibre(object sender, EventArgs e) {
            String text;
            if ((text = BibliotecaAdmin.llibre1.textBoxBuscarLlibre.Text) != "") {
                llibresPopulateBuscador(text);
            } else {
                llibresPopulate();
            }
        }
        ...
        public void autorsPopulate() {
            try {
                ...
                if (llibreGetSelected() != null) {
                    ...
                    if (!BibliotecaAdmin.modificarLlibre1.checkBoxSwitchAutors.Checked) {
        ...
        protected void switchLlibres(object sender, EventArgs args) {
            if (BibliotecaAdmin.modificarAutor1.checkBoxSwitchLlibres.Checked) {
                BibliotecaAdmin.modificarAutor1.buttonEliminarLlibres.BringToFront();

            } else {
                BibliotecaAdmin.modificarAutor1.buttonAfegirLlibres.BringToFront();
            }
            llibresPopulate();
        }
        ...
        protected void eliminarLlibreToAutor(object sender, EventArgs args) {
            if (BibliotecaAdmin.modificarAutor1.dgvLlibres.SelectedRows[0].Cells != null) {
                int id = (autorGetSelected().Id);
                LlibreDTO lDTO = new LlibreDTO(BibliotecaAdmin.modificarAutor1.dgvLlibres.SelectedRows[0].Cells);
                Model.Llibre l = db.Llibre.Where(x => x.Isbn == lDTO.Isbn).FirstOrDefault();
        ...
        public void autorsPopulateBuscador(String text) {
            BibliotecaAdmin.autor1.dgvAutors.DataSource = db.Autor.ToList().Where(a => a.nom.Contains(text) || a.cognoms.Contains(text)).Select(a => new AutorDTO(a)).ToList();
        }
        ...
        public void llibresPopulateBuscador(String text) {
            BibliotecaAdmin.llibre1.dgvLlibres.DataSource = db.Llibre.ToList().Where(a => a.titol.Contains(text)).Select(l => new LlibreDTO(l)).ToList();
        }
        ...
        protected void switchAutors(object sender, EventArgs args) {
            if (BibliotecaAdmin.modificarLlibre1.checkBoxSwitchAutors.Checked) {
                BibliotecaAdmin.modificarLlibre1.buttonEliminarAutors.BringToFront();

            } else {
                BibliotecaAdmin.modificarLlibre1.buttonAfegirAutors.BringToFront();
            }
            autorsPopulate();
        }
        ...
        protected void eliminarAutorToLlibre(object sender, EventArgs args) {
            if (BibliotecaAdmin.modificarLlibre1.dgvAutors.SelectedRows[0].Cells != null) {
                string isbn = (llibreGetSelected().Isbn);
                AutorDTO aDTO = new AutorDTO(BibliotecaAdmin.modificarLlibre1.dgvAutors.SelectedRows[0].Cells);
                Model.Autor a = db.Autor.Where(x => x.Id == aDTO.Id).FirstOrDefault();
            ...
            if (sDTO != null && lDTO != null) {
                ...
                if (cont <= maximDePrestecs) {
                    foreach (var copia in db.Copia.Where(y => y.LlibreIsbn.Equals(llibre.Isbn))) {
                        foreach (var llistaEspera in db.LlistaEspera.Where(yy => yy.LlibreIsbn.Equals(llibre.Isbn) && yy.dataFinalGuardar < DateTime.Today).ToList()) {
                            if (llistaEspera != null) {
                    ...
                    if (c == null) {
                        ...
                        foreach (var llistaEsperatest in db.LlistaEspera.Where(k => k.idSoci == soci.Id && (k.LlibreIsbn.Equals(llibre.Isbn))).ToList()) {
        ...
        protected void finalitzarPrestec(object sender, EventArgs e) {
            ...
            if (prestecDTO != null) {
                ...
                if (db.LlistaEspera.FirstOrDefault(x => x.LlibreIsbn == c.LlibreIsbn) != null) {
                    LlistaEspera l = db.LlistaEspera.FirstOrDefault(x => x.LlibreIsbn == c.LlibreIsbn);
                    ...
                    db.Email.Add(email);
                    MessageBox.Show("Email enviat correctament");
        ...
        private void ActualitzarEmails() {
            string llibreIsbn;
            foreach (var email in db.Email.Where(zz => zz.data > DateTime.Today)) {
                llibreIsbn = db.LlistaEspera.FirstOrDefault(aa => aa.Id == email.Id).LlibreIsbn;

                LlistaEspera l = db.LlistaEspera.FirstOrDefault(x => x.LlibreIsbn == llibreIsbn);
                db.Email.Remove(email);
                db.LlistaEspera.Remove(l);

                llibreIsbn = db.LlistaEspera.FirstOrDefault(aa => aa.Id == email.Id).LlibreIsbn;
                l = db.LlistaEspera.FirstOrDefault(x => x.LlibreIsbn == llibreIsbn);
        ...
        private void veureUsuarisBaixa(object sender, EventArgs e) {
            ...
            if (BibliotecaAdmin.usuari1.checkBoxMostrarBaixa.Checked) {
                var UsuarisBaixa = db.Soci.ToList().Where(s => s.dataBaixa != null).Select(s => new SociDTO(s)).ToList();
                BibliotecaAdmin.usuari1.dgvUsuaris.DataSource = UsuarisBaixa;
